...
public class OCRecipeProvider extends RecipeProvider {
    ...
    private static void rawOreRecipes(Consumer<FinishedRecipe> exporter, RegistryObject<Item> raw, RegistryObject<Item> rawBlock, TagKey<Item> ingot, String name) {
        rawOre(exporter, raw, rawBlock);
        rawOreBlock(exporter, raw, rawBlock);
        ingotFromRaw(exporter, raw, ingot, name);
    }

    private static void rawOre(Consumer<FinishedRecipe> exporter, RegistryObject<Item> raw, RegistryObject<Item> rawBlock) {
        ShapelessRecipeBuilder.shapeless(raw::get, 9).requires(rawBlock::get).unlockedBy("has_raw_ore_block", has(rawBlock::get)).save(exporter);
    }

    private static void rawOreBlock(Consumer<FinishedRecipe> exporter, RegistryObject<Item> raw, RegistryObject<Item> rawBlock) {
        ShapedRecipeBuilder.shaped(rawBlock::get).define('#', raw::get).pattern("###").pattern("###").pattern("###").unlockedBy("has_raw_ore", has(raw::get)).save(exporter);
    }

    private static void ingotFromRaw(Consumer<FinishedRecipe> exporter, RegistryObject<Item> raw, TagKey<Item> ingot, String name) {
